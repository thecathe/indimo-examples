...
%% 
init({Rate, Tick}) ->
    log(init, started, io_lib:format("  rate=~w msg/t   tick_multi=~w", [Rate, Tick])),
    Data = new_data(Rate, Tick),
    % start waiting for first tick
    {ok, waiting, Data, tick(Data)}.

%% tick = Tick / Rate
tick(Data) -> {{timeout, tick}, interval(Data), open}.

%% on tick open -> consume
handle_event({timeout, tick}, open, _State = waiting, Data = #data{tick_count = T}) ->
    {next_state, consume, Data#data{tick_count = T + 1}};

%% consume work -> wait for next tick
...
%% postpone work outside of consume time
