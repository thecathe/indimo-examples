...
%%
%% This is family 8's copy with ONE addition -- pulses/2,3, at the bottom.
%% Copies are meant to diverge exactly like this: family 35 is about the
%% difference between the interval a timer MEASURES and the interval the code
%% MEANS to bound, and the measured one is recoverable from outside by watching
%% when the process runs. Family 8 had no use for that. See the layout note in
%% families/README.md.

-export([vector/1, vector/2, watch/2, watch/3]).
-export([alive/1, blocked/1, spinning/1, mailbox/1, mailbox_empty/1, wait_for/1]).
-export([pulses/2, pulses/3, gaps/1]).
...
%% ---------------------------------------------------------------------------
%% When a process ran, not just whether it ran
%% ---------------------------------------------------------------------------

%% watch/2 collapses a whole window into one bit -- ever_ran. That is the right
%% answer for family 8, where the question is whether a process advanced at all.
%% It is not enough here. A process whose timer is re-armed by every chunk wakes
%% up, does a little work and blocks again, over and over, and the SPACING of
%% those wakeups is the interval its timer is actually measuring.
%%
%% pulses/2,3 keeps the timestamps instead of collapsing them, so the gaps
%% between them can be read off. Note what this does and does not get you: the
%% gaps ARE the measured interval, recovered from outside with no cooperation
%% from the process. The interval the code MEANT to bound is not here and cannot
%% be -- it is not in the code either, only in a comment. See finding 4.
%%
%% Two limits worth knowing. Resolution is PollMs, so a gap is only ever
%% accurate to one poll. And a wakeup is visible only if it costs more than the
%% one reduction the poll itself charges the target; a process that woke, matched
%% nothing and blocked again may not clear that floor.
pulses(Pid, WindowMs) -> pulses(Pid, WindowMs, ?POLL_MS).

%% Returns milliseconds since the window opened, one entry per poll at which the
%% target had done real work since the previous poll.
pulses(Pid, WindowMs, PollMs) ->
    T0 = erlang:monotonic_time(millisecond),
    case snapshot(Pid) of
        dead -> [];
        S0   -> pulse_loop(Pid, T0, T0 + WindowMs, PollMs,
                           maps:get(reductions, S0), [])
    end.

pulse_loop(Pid, T0, Deadline, PollMs, Prev, Acc) ->
    case erlang:monotonic_time(millisecond) >= Deadline of
        true ->
            lists:reverse(Acc);
        false ->
            ...
            case snapshot(Pid) of
                dead ->
                    lists:reverse(Acc);
                S ->
                    R = maps:get(reductions, S),
                    %% One reduction of the gap is this poll's own charge: a
                    %% reading cannot see its own, so it sees exactly the
                    %% previous one's. Anything above that is the target's.
                    Now = erlang:monotonic_time(millisecond),
                    Acc1 = case R - Prev > 1 of
                               true  -> [Now - T0 | Acc];
                               false -> Acc
                           end,
                    pulse_loop(Pid, T0, Deadline, PollMs, R, Acc1)
            end
    end.

%% The intervals between consecutive pulses -- the measured interval, as seen
%% from outside. Fewer than two pulses means the window did not span a full
%% interval, which is itself the answer: nothing to measure.
gaps([])  -> [];
gaps([_]) -> [];
gaps(Pulses) ->
    lists:zipwith(fun(A, B) -> B - A end,
                  lists:droplast(Pulses), tl(Pulses)).
