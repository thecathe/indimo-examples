...
init(Rate) ->
    log(init, started, io_lib:format("  rate=~w msg/s", [Rate])),
    {ok, waiting, #data{rate = Rate, processed = 0, tick_count = 0, backlog = 0}, tick(Rate)}.
...
handle_event(state_timeout, closed, State = consume, Data = #data{tick_count = T}) ->
    ...
    {next_state, waiting, Data2, tick(Data#data.rate)};
...
tick(Rate) -> {{timeout, tick}, interval(Rate), open}.
